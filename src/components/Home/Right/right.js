...
// import TempImg from "../../../assets/images/right-img.jpeg";

export default function Right({ currentUser,setProfilePopupData }) {
  const [typedMessage, setTypesMessage] = useState("");
  const { data } = useContext(ChatContext);
  const typedMessgaeRef = useRef("");
  ...
  return (
    <div className="right-section h-100 position-relative">
      {data.chatId !== null ? (
        <>
          <div className="top-bar d-flex align-items-center">
            <button className="d-flex align-items-center" onClick={()=>setProfilePopupData(data.user)}>
            <img
              src={data.user.photoURL}
              alt={data.user.displayName}
              className="user-img-sm rounded-circle"
            />
            <h2 className="text-orange m-0 ms-3">{data.user.displayName}</h2>              
            </button>

          </div>
          <Messages data={data} />
          <div className="right-bottom-bar position-absolute bottom-0 d-flex w-100 justify-content-between">
            <form className="d-flex w-100" onSubmit={handleSend}>
              <input
                type="text"
                placeholder="Type here.."
                name="message"
                className="message-input bg-grey w-100 px-4"
                value={typedMessage}
                onChange={(e) => {
                  let val = e.target.value;
                  typedMessgaeRef.current = val;
                  setTypesMessage(val);
                }}
              />
              <div className="actions d-flex align-items-center ms-2">
                <button
                  type="button"
                ...
                <EmojiPicker
                  ...
                  open={showEmoji}
                  onEmojiClick={(emoji) => {
                    let str = typedMessgaeRef.current + emoji.emoji;
                    typedMessgaeRef.current = str;
                    setTypesMessage(str);
                  }}
                />
                ...
                <button
                  type="submit"
                  className="send-btn bg-orange rounded-circle"
                >
                  <svg
